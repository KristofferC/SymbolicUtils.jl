...
function count_rewrites(x::Union{DestructuredArgs, Assignment})
    if x isa DestructuredArgs
        if x.create_bindings
            n = 0
            for elem in x.elems
                if elem isa DestructuredArgs && !elem.create_bindings
                    n += length(elem.inds)::Int
                end
            end
            return n
        else
            return length(x.inds)::Int
        end
    elseif x isa Assignment
        lhs = x.lhs
        if lhs isa DestructuredArgs && !lhs.create_bindings
            return length(lhs.inds)::Int
        end
    end
    return 0
end
...
Empty the task-local memoization cache used by [`store_destructuring_rewrites!`](@ref).
Useful for benchmarking codegen without cross-invocation caching effects, analogous to
[`SymbolicUtils.clear_cache!`](@ref) for `@cache`d functions.
"""
function clear_destructuring_rewrites_cache!()
    empty!(DESTRUCTURING_REWRITES_CACHE[])
    return nothing
end

"""
Namespace tag for the marker keys [`store_destructuring_rewrites!`](@ref) leaves in the
`rewrites` dict to record that a given `DestructuredArgs` has already been expanded into
it. Marker keys are `(DESTRUCTURING_MARKER, key)` tuples, which cannot collide with real
rewrite keys (symbolics, `Symbol`s or `Expr`s).
"""
...
function store_destructuring_rewrites!(d::DestructuredArgs, st)
    ...
    # so mark the dict and skip the (identical) re-population on the second call. The
    # `get!` + `length` comparison detects a pre-existing marker with a single lookup.
...
function handle_let_pair!(dargs::Vector{Assignment}, x::Union{DestructuredArgs, Assignment}, st)
    if x isa DestructuredArgs
        if x.create_bindings
            n = count_rewrites(x)
            n > length(st.rewrites) && sizehint!(st.rewrites, length(st.rewrites) + n; shrink = false)
...
@inline function create_array(::Type{A}, S, ::Val{ND}, ::Val{D}, elems...) where {E, N, I, A <: Base.ReshapedArray{E, N, I}, ND, D}
    create_array(I, S, Val(ND), Val(D), elems...)
end
